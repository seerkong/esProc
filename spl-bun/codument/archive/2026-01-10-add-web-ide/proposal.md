...
- Introduce a new `@esproc/spl-flow` package that ports the esProc SPL DSL parsing/execution helpers (Command/ParamParser/KeyWord/cell refs) so cells can author SPL DSL, not just expressions.
...
## Impact
- Specs: update `web-ide` capability to reflect custom grid, vertical step layout, and DSL support; add DSL capability for `@esproc/spl-flow`.
- Code: `packages/web-ide` UI/runtime, new `packages/spl-flow`, docs under `spl-bun/doc`, and tests covering DSL parsing + vertical sheet execution.
