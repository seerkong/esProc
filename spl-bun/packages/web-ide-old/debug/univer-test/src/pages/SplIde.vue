...
// Backend server URL
const API_BASE_URL = "http://localhost:4175";
 ...
 */
function collectColumnA(): string[] {
  if (!workbook) return [];
...
async function runSheet() {
  ...
  try {
    const expressions = collectColumnA();
 ...
 */
function displayResultInGrid(data: { columns: string[]; rows: Record<string, unknown>[] }) {
  if (!gridApi) return;
