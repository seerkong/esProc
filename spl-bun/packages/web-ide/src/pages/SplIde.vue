...
const demos: Demo[] = [
  ...
  {
    id: "flow-if-elseif-else",
    label: "Flow Control: If/ElseIf/Else",
    description: "Branching with indented blocks (B column)",
    cells: [
      { row: 1, col: "A", expr: "x = 0" },
      { row: 2, col: "A", expr: 'branch = ""' },
      { row: 3, col: "A", expr: "if x > 0" },
      { row: 4, col: "B", expr: 'branch = "gt0"' },
      { row: 5, col: "A", expr: "else if x == 0" },
      { row: 6, col: "B", expr: 'branch = "eq0"' },
      { row: 7, col: "A", expr: "else" },
      { row: 8, col: "B", expr: 'branch = "lt0"' },
      { row: 9, col: "A", expr: 'rows = [ { name: "x", value: x }, { name: "branch", value: branch } ]' },
      { row: 10, col: "A", expr: 'out = { columns: ["name", "value"], rows: rows }' },
      { row: 11, col: "A", expr: "out" },
    ],
  },
  {
    id: "flow-for-count",
    label: "Flow Control: For (count)",
    description: "for n iterates 1..n",
    cells: [
      { row: 1, col: "A", expr: "sum = 0" },
      { row: 2, col: "A", expr: "for 3" },
      { row: 3, col: "B", expr: "sum += A2" },
      { row: 4, col: "A", expr: 'rows = [ { name: "sum", value: sum } ]' },
  ...
  {
    id: "flow-for-range",
    label: "Flow Control: For (range)",
    description: "for start,end,step iterates an integer range",
    cells: [
      { row: 1, col: "A", expr: "sum = 0" },
  ...
  {
    id: "flow-for-sequence",
    label: "Flow Control: For (sequence)",
    description: "for sequenceExpr iterates current element",
    cells: [
      { row: 1, col: "A", expr: "sum = 0" },
  ...
  {
    id: "flow-for-while",
    label: "Flow Control: For (while + #seq)",
    description: "for conditionExpr as while-loop; #<cellRef> inside loop",
    cells: [
      ...
      { row: 6, col: "A", expr: 'rows = [ { name: "total", value: total }, { name: "i", value: i } ]' },
      { row: 7, col: "A", expr: 'out = { columns: ["name", "value"], rows: rows }' },
      { row: 8, col: "A", expr: "out" },
    ],
  },
  {
    id: "flow-for-infinite-break",
    label: "Flow Control: For (infinite + break)",
    description: "for loops until break",
    cells: [
      ...
      { row: 6, col: "A", expr: 'rows = [ { name: "count", value: count } ]' },
      { row: 7, col: "A", expr: 'out = { columns: ["name", "value"], rows: rows }' },
      { row: 8, col: "A", expr: "out" },
    ],
  },
  {
    ...
    cells: [
      ...
      { row: 6, col: "A", expr: 'rows = [ { name: "sum", value: sum } ]' },
      { row: 7, col: "A", expr: 'out = { columns: ["name", "value"], rows: rows }' },
      { row: 8, col: "A", expr: "out" },
    ],
  },
  {
    id: "flow-break-target",
    label: "Flow Control: Break (target)",
    description: "break <cellRef> exits an outer loop",
    cells: [
      ...
      { row: 7, col: "A", expr: 'rows = [ { name: "hit", value: hit } ]' },
      { row: 8, col: "A", expr: 'out = { columns: ["name", "value"], rows: rows }' },
      { row: 9, col: "A", expr: "out" },
    ],
  },
  {
    id: "flow-continue-target",
    label: "Flow Control: Continue (target)",
    description: "continue <cellRef> continues an outer loop",
    cells: [
      { row: 1, col: "A", expr: "hit = 0" },
      { row: 2, col: "A", expr: "for 3" },
      { row: 3, col: "B", expr: "for 3" },
  ...
  {
    id: "flow-goto",
    label: "Flow Control: Goto",
    description: "goto jumps to a target cellRef",
    cells: [
      { row: 1, col: "A", expr: "x = 0" },
      { row: 2, col: "A", expr: "goto A4" },
      { row: 3, col: "A", expr: "x = 1" },
      { row: 4, col: "A", expr: "x = 2" },
      { row: 5, col: "A", expr: 'rows = [ { name: "x", value: x } ]' },
      { row: 6, col: "A", expr: 'out = { columns: ["name", "value"], rows: rows }' },
      { row: 7, col: "A", expr: "out" },
    ],
  },
  {
    id: "flow-func-return",
    label: "Flow Control: Func/Return",
    description: "func defines a subroutine; func(A1, ...) invokes it",
    cells: [
  ...
  {
    id: "flow-try",
    label: "Flow Control: Try",
    description: "try captures an error string and continues",
    cells: [
      { row: 1, col: "A", expr: "try" },
      { row: 2, col: "B", expr: "unknownFunc()" },
      { row: 3, col: "A", expr: "ok = 1 + 1" },
      { row: 4, col: "A", expr: 'rows = [ { name: "try", value: A1 }, { name: "ok", value: ok } ]' },
      { row: 5, col: "A", expr: 'out = { columns: ["name", "value"], rows: rows }' },
      { row: 6, col: "A", expr: "out" },
    ],
  },
  {
    id: "flow-result",
    label: "Flow Control: Result",
    description: "result terminates early (UI shows lastQuery before result)",
    cells: [
      { row: 1, col: "A", expr: 'rows = [ { name: "phase", value: "before-result" } ]' },
      { row: 2, col: "A", expr: 'out = { columns: ["name", "value"], rows: rows }' },
      { row: 3, col: "A", expr: "out" },
      { row: 4, col: "A", expr: "result 1" },
      { row: 5, col: "A", expr: 'rows2 = [ { name: "phase", value: "after-result" } ]' },
      { row: 6, col: "A", expr: 'out2 = { columns: ["name", "value"], rows: rows2 }' },
      { row: 7, col: "A", expr: "out2" },
    ],
  },
  {
    id: "flow-end",
    label: "Flow Control: End",
    description: "end terminates with an error message",
    cells: [
      { row: 1, col: "A", expr: 'rows = [ { name: "phase", value: "before-end" } ]' },
      { row: 2, col: "A", expr: 'out = { columns: ["name", "value"], rows: rows }' },
      { row: 3, col: "A", expr: "out" },
